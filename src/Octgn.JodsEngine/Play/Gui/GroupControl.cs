...
namespace Octgn.Play.Gui
{
    ...
    public class GroupControl : UserControlBase
    {
        ...
        protected virtual void OnKeyShortcut(object sender, TableKeyEventArgs e)
        {
            ...
            ActionShortcut match = group.GroupShortcuts.FirstOrDefault(shortcut => shortcut.Key.Matches(e.KeyEventArgs));
            if (match != null)
            {
            ...
            else if (group is Pile pile && pile.ShufflePileShortcut != null && pile.ShufflePileShortcut.Matches(e.KeyEventArgs))
            {
                pile.Shuffle();
                e.Handled = e.KeyEventArgs.Handled = true;
            }
        ...
        protected virtual List<Control> CreateCardMenuItems(Card card, DataNew.Entities.Group def)
        {
            ...
            else
            {
                ...
                if (nCardActions > 0)
                {
                    items.AddRange(actions.Select(action => CreateActionMenuItem(action, CardActionClicked, card)).Where(x => x.IsVisible));
                    if (group.Controller == null)
                        items.Add(new Separator());
                }
                if (group.Controller == null)
        ...
        private MenuItem CreateVisibilityItem()
        {
            ...
            var everybodyItem = new MenuItem
            {
                Header = "Everybody",
                //IsCheckable = true
            };
        ...
        private Control CreateActionMenuItem(IGroupAction baseAction, EventHandler<RoutedEventArgs>  onClick, Card card)
        {
            ...
            if (!showAction) return new MenuItem() { IsVisible = false };
            ...
            {
                foreach (var i in actionGroupDef.Children.Select(subAction => CreateActionMenuItem(subAction, onClick, card)).Where(x => x.IsVisible ))
                    item.Items.Add(i);
                if (item.Items.Count == 0) return new MenuItem() { IsVisible =false};
                return item;
            }
